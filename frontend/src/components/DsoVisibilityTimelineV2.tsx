...
interface DsoVisibilityTimelineV2Props {
  date: string
...
const ASTRO_DARKNESS_LABEL = 'Astronomical darkness'
    ...
    <div className="planet-timeline-track">
      {segments.map((segment) => (
        <span
          ...
          data-testid="dso-timeline-darkness-v2"
          style={{
  ...
  return mergedWindows.flatMap((window) => {
    ...
    return (
      <span
        ...
        data-testid="dso-timeline-segment-v2"
...
export function DsoVisibilityTimelineV2({
  date,
  objects,
  darknessSegments,
  noDarkness = false,
  showNextDaySpilloverHint = false,
}: DsoVisibilityTimelineV2Props) {
  ...
  return (
    <div
      className="planet-timeline dso-timeline dso-timeline-v2"
      data-testid="dso-visibility-timeline-v2"
      data-unified-darkness={unifiedDarkness ? 'true' : 'false'}
      role="img"
      aria-label={`Deep sky visibility timeline for ${date}, 6 PM to 6 AM`}
    >
      <div className="planet-timeline-axis-row dso-timeline-v2-axis-row">
      ...
      <p className="muted planet-timeline-note dso-timeline-v2-footnote">
...
interface DsoTimelineLegendV2Props {
  objects: DsoVisibilityRow[]
  darknessSegments: TimelineSegment[]
}

export function DsoTimelineLegendV2({ objects, darknessSegments }: DsoTimelineLegendV2Props) {
  const unifiedDarkness = forecastAndAstroDarknessFullyOverlap(objects, darknessSegments)

  if (unifiedDarkness) {
    return (
      <div className="planet-timeline-legend" data-testid="dso-visibility-legend-v2">
        <span className="planet-timeline-legend-item">
          <span
            className="planet-timeline-swatch planet-timeline-swatch--astronomical"
            aria-hidden="true"
          />
          {UNIFIED_DARKNESS_LABEL}
        </span>
      </div>
    )
  }
    ...
    <div className="planet-timeline-legend" data-testid="dso-visibility-legend-v2">
      ...
      <span className="planet-timeline-legend-item">
        <span
          ...
          aria-hidden="true"
        />
        {FORECAST_DARKNESS_LABEL}
      </span>
    </div>
  )
}

export function DsoDayDetailsV2({ objects }: { objects: DsoVisibilityRow[] }) {
  return (
    <ul className="planet-timeline-details dso-timeline-v2-details" data-testid="dso-visibility-details-v2">
      {objects.map((row) => (
        <li key={row.id} className="planet-timeline-detail-row" data-testid="dso-visibility-detail-v2">
